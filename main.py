...
def generate_and_serve_map(target_name):
    ...
    # 4. 서버 실행 (이미 켜져있지 않을 때만 실행)
    if not server_started:
        def run_server():
            Handler = http.server.SimpleHTTPRequestHandler
            # 포트 재사용 허용 (서버 재시작 시 에러 방지)
        ...
        # daemon=True로 설정하면 메인 프로그램(GUI)이 꺼질 때 서버 스레드도 같이 꺼집니다.
        server_thread = threading.Thread(target=run_server, daemon=True)
        server_thread.start()

        server_started = True  # 서버가 시작되었음을 표시
    else:
